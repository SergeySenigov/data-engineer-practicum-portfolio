...
@dag(
    schedule_interval='0/15 * * * *',  # Задаю расписание выполнения дага - каждый 15 минут.
...
def sprint5_example_stg_init_schema_dag():
    ...
    # Забираю путь до каталога с SQL-файлами из переменных Airflow.
    ddl_path = Variable.get("EXAMPLE_STG_DDL_FILES_PATH")

    # Объявляю таск, который создает структуру таблиц.
    ...
    # Задаю последовательность выполнения тасков - только инициализация схемы.
    init_schema  


# Вызываю функцию, описывающую даг.
